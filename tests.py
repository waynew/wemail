...
def test_if_draft_save_is_called_with_different_name_new_should_exist_old_should_be_gone(testdir):
    with wemail.Message(draftdir=testdir) as draft:
        filename = draft.filename
...
def test_replyify_should_keep_attachments_if_True(good_draft):
    ...
    msg = wemail.replyify(msg=original_msg, keep_attachments=True, sender='foo@example.com')
...
def test_replyify_should_strip_attachments_if_keep_is_False():
    ...
    msg = wemail.replyify(msg=original_msg, keep_attachments=False, sender='foo@example.com')
...
def test_replyify_should_set_to_and_cc_to_originals_if_reply_all():
    ...
    expected_to = getaddresses(original_msg.get_all('From') + original_msg.get_all('To'))
...
def test_replyify_should_use_reply_to_if_it_exists():
    ...
    assert getaddresses(msg.get_all('To')) == getaddresses(original_msg.get_all('Reply-To'))
...
def test_forwardify_should_keep_attachments_if_True(good_draft):
    ...
    msg = wemail.forwardify(msg=original_msg, keep_attachments=True, sender='foo@example.com')
...
def test_forwardify_should_strip_attachments_if_keep_is_False():
    ...
    msg = wemail.forwardify(msg=original_msg, keep_attachments=False, sender='foo@example.com')
